...
function ProjectCard({ project, featured = false }: { project: typeof projects[number]; featured?: boolean }) {
  return <a href={project.href} className={`group relative block min-h-56 overflow-hidden bg-[#26374a] shadow-xl shadow-black/15 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-cream ${featured ? "lg:h-[420px]" : "lg:h-[204px]"}`}>
    ...
    <span className="absolute inset-x-0 bottom-0 flex items-end justify-between gap-4 p-5 text-white">
      <span><span className="block text-base font-semibold leading-tight sm:text-lg">{project.title}</span><span className="mt-1 block text-xs text-white/80 sm:text-sm">{project.location}</span></span>
...
export default function FeaturedProjects() {
  return <section id="projects" aria-labelledby="projects-heading" className="bg-[radial-gradient(circle_at_48%_40%,#233b58_0%,#14263e_58%,#102037_100%)] px-5 py-12 text-white sm:px-8 lg:px-12 lg:py-7">
    <div className="mx-auto grid max-w-[1450px] gap-8 lg:grid-cols-[minmax(230px,0.8fr)_minmax(0,1.15fr)_minmax(0,0.88fr)] lg:gap-5">
      <div className="flex flex-col items-start justify-center lg:pr-7">
        ...
        <h2 id="projects-heading" className="max-w-sm text-3xl font-semibold leading-[1.13] tracking-tight sm:text-4xl lg:text-[clamp(28px,2.5vw,42px)]">Building Excellence<br />Across Every Project</h2>
      ...
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-1"><ProjectCard project={projects[1]} /><ProjectCard project={projects[2]} /></div>
    </div>
  </section>;
}
