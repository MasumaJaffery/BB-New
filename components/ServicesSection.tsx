...
export default function ServicesSection() {
  return <section aria-labelledby="services-heading" className="bg-white px-5 py-16 text-[#14263e] sm:px-8 lg:py-20">
    <div className="mx-auto max-w-[1450px]">
      <div className="mb-10 text-center">
        <div className="mb-3 flex items-center justify-center gap-3 text-xs font-semibold uppercase tracking-[0.06em]">What We Do<span aria-hidden="true" className="h-px w-8 bg-[#d9bd97]" /></div>
        <h2 id="services-heading" className="text-3xl font-semibold tracking-tight sm:text-4xl">Comprehensive Construction Services</h2>
      </div>
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
        {services.map(service => <article key={service.title} className="flex h-full flex-col border border-[#e8ebef] bg-white shadow-sm shadow-[#17283e]/[0.03]">
          <div className="relative h-40 w-full overflow-hidden sm:h-44 lg:h-40"><Image src={service.image} alt="" fill sizes="(min-width: 1024px) 25vw, (min-width: 640px) 50vw, 100vw" className="object-cover" /></div>
